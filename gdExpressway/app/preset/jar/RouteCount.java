...
/**
 * 统计固定时间内各个路段的车流量 四个参数，第一个数据地址，第二个输出地址，第三个开始时间，第四个结束时间，时间格式YYYYMMDD 输出路段-车流量
 * 
 * @author amber
 * 
 */
public class RouteCount {
		...
		protected void map(LongWritable key, Text value, Context context)
			...
			if (line.length() > 9) {
				...
				if (x <= date && date <= y) {
					...
					int inAreaNo = -1, inEntryNo = -1, outAreaNo = -1, outEntryNo = -1, last = 0;
					int sum = 0, i;
	...
	public static void main(String[] args) throws Exception {
		...
		conf.set("mapred.job.tracker", args[4]);
		conf.set("fs.default.name", args[5]);
		conf.set("Hadoop.job.user", args[6]);
		Job job = new Job(conf, "RouteCount-" + args[6] + "_" + args[7]);
		((JobConf) job.getConfiguration()).setJar("RouteCount.jar");
